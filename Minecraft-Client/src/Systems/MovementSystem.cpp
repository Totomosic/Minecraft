		...
		for (EntityHandle entity : entities.GetEntitiesWith<Transform, CMovement>())
		{
			...
			if (movement->Velocity.MagnitudeSqrd() > 1e-9)
			{
				if (entity.HasComponent<CDimensions>())
				{
					...
					BlockPos_t next = m_World->GetBlockFromWorld(nextPos);
					BlockPos_t xBlock0 = m_World->GetBlockFromWorld(transform->Position() + Vector3f{ move.x + size.x * Sign(move.x), 0, size.z });
					BlockPos_t xBlock1 = m_World->GetBlockFromWorld(transform->Position() + Vector3f{ move.x + size.x * Sign(move.x), 0, -size.z });
					BlockPos_t zBlock0 = m_World->GetBlockFromWorld(transform->Position() + Vector3f{ size.x, 0, move.z + size.z * Sign(move.z) });
					BlockPos_t zBlock1 = m_World->GetBlockFromWorld(transform->Position() + Vector3f{ -size.x, 0, move.z + size.z * Sign(move.z) });
